...
func GetApp(assets embed.FS, wjs string) *App {
	if appOnce == nil {
		...
		appOnce = &App{
			assets:      assets,
			AppName:     "Res Downloader Modified",
			Version:     version,
			Description: "A personalized modified distribution based on res-downloader, with Molly themes, editable pages, custom widgets, and resource download workflows.",
			Copyright:   "Copyright (c) 2026~" + strconv.Itoa(time.Now().Year()) + " houqingxin <houqingxincampus@hotmail.com>. Based on res-downloader; original rights remain with upstream authors. See LICENSE and NOTICE.",
			IsReset:     false,
