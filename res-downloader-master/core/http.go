...
func (h *HttpServer) send(t string, data interface{}) {
	...
	wailsruntime.EventsEmit(appOnce.ctx, "event", string(jsonData))
}
...
func (h *HttpServer) openDirectoryDialog(w http.ResponseWriter, r *http.Request) {
	folder, err := wailsruntime.OpenDirectoryDialog(appOnce.ctx, wailsruntime.OpenDialogOptions{
		DefaultDirectory: "",
...
func (h *HttpServer) openFileDialog(w http.ResponseWriter, r *http.Request) {
	filePath, err := wailsruntime.OpenFileDialog(appOnce.ctx, wailsruntime.OpenDialogOptions{
...
func (h *HttpServer) openImageFileDialog(w http.ResponseWriter, r *http.Request) {
	filePath, err := wailsruntime.OpenFileDialog(appOnce.ctx, wailsruntime.OpenDialogOptions{
		Filters: []wailsruntime.FileFilter{
			{
...
func (h *HttpServer) localImage(w http.ResponseWriter, r *http.Request) {
...
func (h *HttpServer) envCheck(w http.ResponseWriter, r *http.Request) {
	checks := []respData{}
	addCheck := func(key, label string, ok bool, detail string) {
		status := "ok"
		if !ok {
			status = "warn"
		}
		checks = append(checks, respData{
			"key":    key,
			"label":  label,
			"status": status,
			"detail": detail,
	...
	addCheck("platform", "运行平台", appOnce != nil && appOnce.UserDir != "", stdruntime.GOOS+"/"+stdruntime.GOARCH)
	addCheck("service", "本地服务", globalConfig.Host != "" && globalConfig.Port != "", "http://"+globalConfig.Host+":"+globalConfig.Port)
	addCheck("proxy", "系统代理", appOnce.IsProxy, map[bool]string{true: "已启动代理", false: "未启动代理"}[appOnce.IsProxy])
	addCheck("certificate", "证书状态", appOnce.isInstall(), map[bool]string{true: "证书已安装或已完成初始化", false: "证书尚未完成安装"}[appOnce.isInstall()])

	saveDirOk := false
	saveDetail := "未设置保存目录"
	if globalConfig.SaveDirectory != "" {
		if stat, err := os.Stat(globalConfig.SaveDirectory); err == nil && stat.IsDir() {
			saveDirOk = true
			saveDetail = globalConfig.SaveDirectory
		} else {
			saveDetail = "保存目录不可用：" + globalConfig.SaveDirectory
		}
	}
	addCheck("saveDirectory", "保存目录", saveDirOk, saveDetail)

	addCheck("resourceType", "抓取类型", true, "当前类型配置已加载")
